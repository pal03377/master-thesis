...
In this section, we will list concrete hypotheses derived from the general objectives of our thesis.

=== Scalability
While we expect the processing times to increase with the number of submissions and feedback items, we expect ThemisML to be able to handle a large number of submissions and feedback items in a reasonable amount of time.
...
#figure(
  ...
  caption: [Required processing time for processing a feedback item on the first $f$ submissions, given $n$ previous submissions],
...
We will share our subjective findings in @findings.
