...
3. The _Athena Feedback Sending Service_ takes on the role of transmitting new feedback to Athena, enabling Athena to refine its feedback suggestions.
...
When the CoFee Adapter gets a request, it forwards it to the Athena-CoFee server for processing based on the approach developed by Bernius et al.~#cite("cofee", "cofee2").
...
In each experiment, we recorded both the feedback processing time and the feedback suggestion request time.
Because there is a simple check in ThemisML that recognizes two submissions as the same if they are identical ignoring whitespace and we wanted to test the actual processing time of ThemisML without this optimization, we ran the test on a set of non-identical submissions.
To avoid caching effects leaking between experiments, we cleared ThemisML's cache in between each run.
...
The suggestion generation was consistently completed in less than 0.5 seconds, with submission and feedback numbers of all tested sizes (up to 1,000). Given its rapid response time, we decided not to analyze the performance analysis of the suggestion generation endpoint further, as the current speed already meets the operational requirements.

Instead, we focused on the feedback processing endpoint, in which the computationally expensive code comparisons and feedback suggestion preparations take place. We recorded the processing times for the feedback processing endpoint in @evaluationScalability1a, @evaluationScalability1b, @evaluationScalability2, and @evaluationScalability3.
#figure(
...
) <evaluationScalability1a>
#figure(
  image("figures/scalability-1b.png", width: 90%),
  caption: [Required processing time for processing a feedback item on each of the first $f$ submissions, given $n=100$ previous submissions],
) <evaluationScalability1b>
#figure(
...
=== Design
// Note: Describe the design / methodology of the evaluation and why you did it like that. E.g. what kind of evaluation have you done (e.g. questionnaire, personal interviews, simulation, quantitative analysis of metrics, what kind of participants, what kind of questions, what was the procedure?)
// ?? Add more detail and why I did it like that!
ThemisML's primary goal is to provide quality feedback. We conducted a series of tests to determine its feedback accuracy:
...
To test these scenarios, we had to temporarily disable a feature in ThemisML that detects feedback that affects too many other submissions. // ?? Describe more about why
...
The three exercises we tested ThemisML on are:
1. ID 3184: *Objektorientierung mit Verkehrsmitteln* (Java, WS20/21), using random 50 submissions for training and another random 50 submissions for testing (out of 756 submissions available)
2. ID 3908: *Aktor-Threads* (Java, WS20/21), using random 40 submissions for training and another random 40 submissions for testing (out of 81 submissions available)
3. ID 3913: *Shoppende Pinguine* (Java, WS20/21), using random 48 submissions for training and another random 48 submissions for testing (out of 97 submissions available)

We specifically used these exercises because we already had an export of all submissions and feedback available from a data export by Dietrich~@athenaLLMs.

=== Objectives
// Note: Derive concrete objectives / hypotheses for this evaluation from the general ones in the introduction.
We expect ThemisML to provide suggestions that are similar to the manual feedback provided by tutors.
ThemisML should only provide suggestions that tutors will accept most of the time.

We will gain insights into the quality of the suggestions by manually reviewing them and comparing them to the manual feedback provided by tutors. This way, we get a subjective overview of the quality of the suggestions.
...
#figure(
  table(
    columns: (8em, 6em, auto, auto, auto, auto),
    inset: 10pt,
    align: left,
    [*Feedback by*], [*Exercise*], [*Min*], [*Max*], [*Avg*], [*Median*],

    // Suggestions for Exercise 1 by ThemisML
    [ThemisML],
    [1],
    [0],
    [5],
    ...
    // Suggestions for Exercise 2 by ThemisML
    [ThemisML],
    [2],
    [0],
    [3],
    ...
    // Suggestions for Exercise 3 by ThemisML
    [ThemisML],
    [3],
    [1],
    [8],
    ...
    // Feedback for Exercise 1 by Tutors
    [Tutors],
    [1],
    [2],
    [54],
    ...
    // Feedback for Exercise 2 by Tutors
    [Tutors],
    [2],
    [5],
    [29],
    ...
    // Feedback for Exercise 3 by Tutors
    [Tutors],
    [3],
    [3],
    [61],
